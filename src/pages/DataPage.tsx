import { Database, Download, FileSpreadsheet, FlaskConical, HardDrive, ShieldCheck, Trash2, Upload } from 'lucide-react'
...
function Card({ icon: Icon, title, children }: { icon: typeof Database; title: string; children: ReactNode }) {
  return (
    <section className="card p-5">
      <h2 className="mb-3 flex items-center gap-2 font-semibold text-slate-900"><Icon className="h-4 w-4 text-slate-500" aria-hidden />{title}</h2>
      {children}
    </section>
  )
}
  ...
  return (
    <details className="rounded-lg border border-slate-200">
      ...
      <div className="border-t border-slate-200 p-3">
        ...
        <pre className="overflow-x-auto rounded-lg bg-slate-900 p-3 text-xs leading-relaxed text-slate-100">{code}</pre>
      </div>
    </details>
  )
}
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <Card icon={HardDrive} title="Espace utilisé">
        <dl className="grid gap-3 text-sm sm:grid-cols-3">
          <div><dt className="text-slate-500">Données (offres, profil…)</dt><dd className="font-semibold text-slate-900">{formatBytes(jsonSize)}</dd></div>
          <div><dt className="text-slate-500">Fichiers ({data.files.length})</dt><dd className="font-semibold text-slate-900">{formatBytes(filesSize)}</dd></div>
          <div><dt className="text-slate-500">Total navigateur (estimation)</dt><dd className="font-semibold text-slate-900">{usage ? `${formatBytes(usage.used)} / ${formatBytes(usage.quota)}` : 'Indisponible'}</dd></div>
        </dl>
      </Card>
      ...
      <Card icon={FlaskConical} title="Données de démonstration">
        <p className="mb-3 text-sm text-slate-600">Ajoute 5 entreprises et 10 offres fictives (statuts, scores et relances variés) pour tester l'application. Elles s'ajoutent à vos données ; supprimez-les depuis les tableaux.</p>
      ...
      <Card icon={Trash2} title="Zone de danger">
        <div className="flex flex-wrap gap-2">
          <button className="btn-secondary text-rose-700" onClick={() => setConfirm('files')} disabled={!data.files.length}>Supprimer uniquement les fichiers</button>
      ...
      <Card icon={Database} title="Format JSON attendu de Claude">
        <p className="mb-3 text-sm text-slate-600">Les prompts demandent déjà ce format à Claude. Les clés absentes sont tolérées ; les dates sont au format YYYY-MM-DD ; le score va de 0 à 100. La détection des doublons se fait par URL exacte, URL nettoyée (sans paramètres de suivi), puis entreprise + intitulé + lieu.</p>
