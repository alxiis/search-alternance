      ...
      <div className="flex h-36 items-end gap-2" role="img" aria-label={`Candidatures envoyées par semaine sur ${WEEKS} semaines : ${counts.map((c) => c.value).join(', ')}`}>
        {counts.map((c) => (
          <div key={c.label} className="flex flex-1 flex-col items-center justify-end gap-1">
            ...
            <div className="w-full rounded-t bg-primary" style={{ height: `${(c.value / max) * 100}%`, minHeight: c.value ? 4 : 2, opacity: c.value ? 1 : 0.2 }} />
          </div>
        ))}
      </div>
      <div className="mt-1 flex gap-2">
        {counts.map((c) => <span key={c.label} className="flex-1 text-center text-[10px] text-muted-foreground">{c.label}</span>)}
      </div>
    </div>
  )
}
...
function Shortcut({ to, icon: Icon, label, onClick }: { to?: string; icon: LucideIcon; label: string; onClick?: () => void }) {
  const cls = 'flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5 text-sm font-medium text-foreground hover:border-ring hover:text-accent-foreground'
  ...
  return (
    <div className="space-y-6">
      {(!profileReady || !jobs.length) && (
        <section className="card border-accent bg-accent p-5">
          <h2 className="font-semibold text-foreground">Pour démarrer</h2>
          <ol className="mt-2 list-decimal space-y-1 pl-5 text-sm text-foreground">
            <li><Link className="font-medium text-accent-foreground hover:underline" to="/profil">Renseignez votre profil</Link> et importez votre CV.</li>
            <li><Link className="font-medium text-accent-foreground hover:underline" to="/criteres">Définissez vos critères</Link> de recherche.</li>
            <li><Link className="font-medium text-accent-foreground hover:underline" to="/recherche">Générez un prompt</Link>, collez-le dans Claude, puis importez le JSON obtenu.</li>
          </ol>
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-3 2xl:grid-cols-6" aria-label="Statistiques">
        <StatCard label="Offres au total" value={stats.total} icon={Briefcase} to="/offres" />
        <StatCard label="À candidater" value={stats.toApply} icon={CalendarClock} tone="sky" to="/kanban" />
        <StatCard label="Candidatures envoyées" value={stats.sent} icon={Send} tone="violet" to="/offres" />
        <StatCard label="Entretiens" value={stats.interviews} icon={MessagesSquare} tone="teal" to="/kanban" />
        <StatCard label="Refus" value={stats.rejected} icon={XCircle} tone="rose" to="/kanban" />
        <StatCard label="Acceptées" value={stats.accepted} icon={CheckCircle2} tone="emerald" to="/kanban" />
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="card min-w-0 p-5">
          <h2 className="mb-4 font-semibold text-foreground">Prochaines relances et entretiens</h2>
          {tasks.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">Aucune relance prévue. Une date de relance est proposée automatiquement quand vous passez une offre en « Postulé ».</p>
          ) : (
              ...
              {tasks.map((t) => {
                ...
                return (
                  <li key={`${t.jobId}-${t.kind}`}>
                    <button className="flex w-full items-center gap-3 py-2.5 text-left hover:bg-muted" onClick={() => openJob(job.id)}>
                      <span className={cn('w-24 shrink-0 text-xs font-medium', t.overdue ? 'text-danger' : 'text-muted-foreground')}>
                        {formatDate(t.dueDate).replace(/ \d{4}$/, '')}{t.overdue ? ' · en retard' : ''}
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-sm font-medium text-foreground">{names.get(job.companyId ?? '') ?? 'Entreprise inconnue'} — {job.title || 'Offre'}</span>
                        <span className="text-xs text-muted-foreground">{t.label}{job.application.nextAction ? ` · ${job.application.nextAction}` : ''}</span>
                      </span>
                    </button>
                  </li>
                )
        ...
        <section className="card min-w-0 p-5">
          <h2 className="mb-4 font-semibold text-foreground">Candidatures envoyées par semaine</h2>
          <WeeklyChart counts={weekly} />
        </section>
      </div>

      <section className="card">
        <h2 className="px-5 pt-5 font-semibold text-foreground">Dernières candidatures</h2>
        {latest.length === 0 ? (
          ...
          <ul className="divide-y divide-border p-2">
            {latest.map((j) => (
              <li key={j.id}>
                <button className="flex w-full flex-wrap items-center gap-x-4 gap-y-1 rounded-lg px-3 py-2.5 text-left hover:bg-muted" onClick={() => openJob(j.id)}>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-foreground">{j.title || 'Offre'}</span>
                    <span className="text-xs text-muted-foreground">{names.get(j.companyId ?? '') ?? 'Entreprise inconnue'} · le {formatDate(j.application.appliedAt)}</span>
                  </span>
